...
export function analyzeTokenMomentum(token: Token): AdvisorSignal {
  ...
  // Calculs intermédiaires mathématiques
  ...
  // Variables collectrices pour le moteur de diagnostic
  ...
  if (pnl >= 20 && m.volume24h > 10000000 && deltaVol1h > 25 && athDrop <= 15) {
    ...
    color = '#00ffcc'; // Vert néon d'enfer
  ...
  // SCÉNARIO 2 : RÈGLE D'OR DU DEGEN (Prioritaire si PnL massif)
  else if (pnl >= 100) {
    type = 'GOLDEN_RULE';
  ...
  // SCÉNARIO 4 : PIÈGE DE LA LIQUIDITÉ
  else if (athDrop <= 12 && deltaVol1h < -40 && ratioVolLiq < 0.15) {
    type = 'LIQUIDITY_TRAP';
    label = 'Piège de la Liquidité';
    color = '#ef4444'; // Rouge standard
    badgeBg = 'rgba(239, 68, 68, 0.1)';
    icon = 'ShieldAlert';
    explanation = "Danger : Le prix est artificiellement maintenu proche de l'ATH mais le volume s'est effondré. Risque d'impossibilité de sortie de taille.";
    reasons = ['Chute drastique de la vélocité des échanges.', `Ratio Volume/Liquidité excessivement faible (${ratioVolLiq.toFixed(3)}).`];
    confidence = 'HIGH';
  }
  // SCÉNARIO 3 : REPLI SAIN EN BULL MARKET
    ...
    icon = 'TrendingUp';
    explanation = "Excellente opportunité d'accumulation (DCA) si le narratif fondamental reste intact.";
    reasons = [`Correction technique saine de ${athDrop.toFixed(0)}% depuis l'ATH.`, 'La structure de volume montre une résilience manifeste des acheteurs.'];
    confidence = 'MEDIUM';
  }
  // SCÉNARIO 5 : CHOC DE VOLUMES HORAIRES
    ...
    color = '#a855f7'; // Violet électrique
    badgeBg = 'rgba(168, 85, 247, 0.1)';
    icon = 'Zap';
    explanation = "Une anomalie de volume horaire a été détectée. Une entrée d'insiders ou une news majeure est imminente.";
    reasons = [`Augmentation soudaine de +${deltaVol1h.toFixed(0)}% du volume sur les 60 dernières minutes.`];
    confidence = 'MEDIUM';
  }
  // SCÉNARIO 6 : CAPITULATION
  else if (pnl <= -40 || athDrop > 60) {
    const isDead = deltaVol1h < -50 && m.volume24h < 20000;
    if (isDead) {
      type = 'DEAD_TOKEN';
      label = 'Dead Token / Rug';
      color = '#7f1d1d'; // Rouge sang très sombre
      badgeBg = 'rgba(127, 29, 29, 0.2)';
      icon = 'Skull';
      explanation = 'Le projet ne montre plus aucun signe d\'activité on-chain ou d\'intérêt spéculatif. Vente à perte recommandée pour recycler la poussière.';
      reasons = ['Volume 24h squelettique.', 'Perte de valeur irréversible proche de 99% du sommet.'];
      confidence = 'HIGH';
    } else {
      type = 'CAPITULATION';
      label = 'Capitulation';
      ...
      icon = 'TrendingDown';
      explanation = 'Vente panique globale des retail holders. Surveillez l\'apparition d\'un bottom de volume (Smart Money Entry).';
      reasons = [`Chute sévère de -${athDrop.toFixed(0)}% par rapport aux plus hauts historiques.`];
      confidence = 'MEDIUM';
    }
  }
  // SCÉNARIO EXTENSION : WHALE ACCUMULATION
    ...
    icon = 'Anchor';
    explanation = 'Les grosses baleines (top-wallets) accumulent agressivement le supply disponible hors des exchanges.';
