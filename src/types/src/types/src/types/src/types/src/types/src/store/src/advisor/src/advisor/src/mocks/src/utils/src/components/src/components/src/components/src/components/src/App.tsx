...
export default function App() {
  ...
  // Déclenche un rafraîchissement initial automatique au montage de la page
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 selection:bg-cyan-500/30 selection:text-cyan-200 antialiased flex flex-col">
      {/* Barre de navigation premium et monitoring global des fonds */}
      <Header />

      {/* Zone principale de travail (Layout Grid adaptatif) */}
      <main className="flex-1 max-w-7xl w-full mx-auto p-4 md:p-6 flex flex-col gap-6">
        
        {/* Ligne 1 : Saisie de données publiques des wallets Solana / Base */}
        <WalletInput />

        {/* Ligne 2 : Moteur analytique prédictif Degen Advisor */}
        <AdvisorCard />

        {/* Ligne 3 : Vue d'ensemble du tableau de bord complet style DexScreener */}
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between px-1">
            ...
            <div className="hidden sm:flex items-center gap-1.5 text-[10px] font-mono text-slate-600">
              ...
              <span>Double-cliquez ou survolez "Avg Entry" pour modifier vos prix d'achat</span>
            </div>
          </div>
